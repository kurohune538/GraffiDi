...
// The number for the brightest moment
// TODO: I want to calibrate this number
int max_range = 200;
...
//      }
      ...
      Serial.print(" 3 : ");
      Serial.println(readMux3(i - 20));
      current_value[i] = readMux3(i - 20);
//      if (current_value[i] > calibrated_number[i] + hys) {
//        strip2.setPixelColor(i - 20, strip2.Color(0, 255 * current_value[i] / (calibrated_number[i] + max_range), 0));
//      }
    } else if (i >= 28) {
      Serial.print(" 2' : ");
      Serial.println(readMux2(i - 28 + 4));
      current_value[i] = readMux2(i - 28 + 4);
//      if (current_value[i] > calibrated_number[i] + hys) {
//        strip2.setPixelColor(i - 20, strip2.Color(0, 255 * current_value[i] / (calibrated_number[i] + max_range), 0));
//      }
    }
...
//    strip2.show();
  ...
  for (int i = 0; i < sensor_count; i++) {
    ...
    if (i < 20) {
      ...
      if (current_value[i] > calibrated_number[i] + hys) {
        strip2.setPixelColor(i, strip2.Color(0, 255 * current_value[i] / (calibrated_number[i] + max_range), 0));
